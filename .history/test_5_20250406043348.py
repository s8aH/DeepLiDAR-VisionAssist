...
def main(args):
    ...
    def play_audio_in_thread(filename):
        """Plays audio in a dedicated thread and cleans up"""
        def _play():
            try:
                playsound(filename)
            finally:
        ...
        thread.start()
        return thread  # Return thread in case you want to track it

    # --- Usage in your code ---
    if args.generate_audio:
        audio_file = "temp_announce.mp3"
        generate_audio(results, CLASSES, num_samples=5, audio_filename=audio_file)
        play_audio_in_thread(audio_file)  # Starts playing immediately in background
    ...
    cv2.waitKey(0)  # Main thread handles windows
