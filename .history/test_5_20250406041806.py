...
def main(args):
    ...
    def show_image_window(annotated, window_name):
        cv2.imshow(window_name, annotated)
        print("Image window opened")
        cv2.waitKey(0)  # Wait until any key is pressed
        cv2.destroyAllWindows()

    def show_point_cloud(pc, bboxes, labels):
        try:
            print("Launching point cloud viewer...")
            vis_pc(pc, bboxes=bboxes, labels=labels)  # This blocks until closed
        except Exception as e:
            print(f"Error visualizing point cloud: {e}")

    def play_audio(results, classes):
        print("Playing audio")
        generate_audio(results, classes, num_samples=5)

# Your existing code to get `results` and `annotated`
    if image is not None:
        results, annotated = annotate_image(
            image, results, CLASSES,
            limited_distance=args.limited_distance,
        ...
        # Give it a moment to flush disk/save
        time.sleep(0.5)
    ...
    if image is not None:
        img_thread = threading.Thread(
            ...
            args=(annotated, f'{os.path.basename(args.img_path)}-3d bbox'),
            daemon=True
        )
        threads.append(img_thread)

    # --- Point Cloud Thread ---
    pc_thread = threading.Thread(
        target=show_point_cloud,
        args=(pc, results['lidar_bboxes'], results['labels']),
        daemon=True
    )
    ...
    # Keep main thread alive until all non-daemon threads finish
    for t in threads:
        t.join()
