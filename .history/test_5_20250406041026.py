...
def main(args):
    ...
    def show_point_cloud(pc, bboxes, labels):
        try:
            vis_pc(pc, bboxes=bboxes, labels=labels)
        except Exception as e:
            print(f"Error visualizing point cloud: {e}")
    ...
    if image is not None:
        ...
        image_thread = threading.Thread(target=show_image_window, args=(annotated, f'{os.path.basename(args.img_path)}-3d bbox'))
        threads.append(image_thread)

    # Point cloud visualization
    pc_thread = threading.Thread(target=show_point_cloud, args=(pc, results['lidar_bboxes'], results['labels']))
    threads.append(pc_thread)

    # Audio playback
    if args.generate_audio:
        audio_thread = threading.Thread(target=play_audio, args=(results, CLASSES))
