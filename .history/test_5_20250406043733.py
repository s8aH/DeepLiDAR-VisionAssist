...
def main(args):
    ...
    #     results, annotated = annotate_image(
    #         image, results, CLASSES,
    ...
    #         generate_audio(results, CLASSES, num_samples=5)

    import subprocess
    import os
    from pathlib import Path

    def truly_non_blocking_play(audio_file):
        """Plays audio without ANY UI blocking"""
        if not Path(audio_file).exists():
            return
        
        # Platform-specific players that won't block
        if os.name == 'nt':  # Windows
            subprocess.Popen(['cmd', '/c', 'start', '/MIN', audio_file], 
                            shell=True,
                            stdin=subprocess.DEVNULL,
                            stdout=subprocess.DEVNULL,
                            stderr=subprocess.DEVNULL)
        elif os.name == 'posix':  # Mac/Linux
            subprocess.Popen(['xdg-open', audio_file] if os.uname().sysname != 'Darwin' else ['afplay', audio_file],
                            stdin=subprocess.DEVNULL,
                            stdout=subprocess.DEVNULL,
                            stderr=subprocess.DEVNULL)
        
        # Clean up after 5 seconds (adjust as needed)
        threading.Timer(5.0, lambda: os.remove(audio_file) if os.path.exists(audio_file) else None).start()

    # In your visualization code:
    if args.generate_audio:
        audio_file = "announce.mp3"
        generate_audio(results, CLASSES, num_samples=5, audio_filename=audio_file)
        truly_non_blocking_play(audio_file)  # THIS WON'T BLOCK ANYTHING

    # Your existing window code
    cv2.imshow('Window', image)
    vis_pc(pc)  # This will now work while audio plays
    cv2.waitKey(0)
