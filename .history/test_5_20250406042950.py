...
def main(args):
    ...
    def play_audio_async(filename):
        """Play audio in a non-blocking way using threading"""
        def play():
            playsound(filename)
            os.remove(filename)
    ...
    # Generate and play audio ASYNC
    if args.generate_audio:
        audio_file = "temp_audio.mp3"
        generate_audio(results, CLASSES, num_samples=5, audio_filename=audio_file)
        play_audio_async(audio_file)  # Non-blocking playback

    # Show point cloud (blocking operation)
    try:
        vis_pc(pc, bboxes=results['lidar_bboxes'], labels=results['labels'])
    except Exception as e:
        print(f"Error visualizing point cloud: {e}")
