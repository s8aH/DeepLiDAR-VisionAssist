...
def main(args):
    ...
    def generate_and_play_audio(results, classes, num_samples=5):
        """Thread target function to generate and play audio without blocking."""
        audio_filename = "temp_announcement.mp3"
        announcements = generate_audio(results, classes, num_samples, audio_filename)
        playsound(audio_filename)  # Will play while windows are open
        os.remove(audio_filename)
    ...
    if args.generate_audio:
        audio_thread = threading.Thread(
            ...
            args=(results, CLASSES, 5)
        )
        audio_thread.daemon = True  # Terminate thread if main exits
        audio_thread.start()

    # --- Point Cloud Visualization (Blocking) ---
    try:
        vis_pc(pc, bboxes=results['lidar_bboxes'], labels=results['labels'])
    except Exception as e:
        print(f"Error visualizing point cloud: {e}")
    ...
    cv2.waitKey(0)  # Blocks here until windows are closed
